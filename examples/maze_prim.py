...
def generate_maze_prims():
    ...
    start_r = random.randrange(0, grid_height // 2) * 2 # Start on an even coordinate
    start_c = random.randrange(0, grid_width // 2) * 2
    maze[start_r][start_c] = PATH_CHAR
    grid.set_color(start_c, start_r, PATH_COLOR)

    # Frontier stores tuples: (neighbor_r, neighbor_c, origin_r, origin_c)
    ...
    while frontier:
        ...
        fr_r, fr_c, origin_r, origin_c = random.choice(frontier)
        frontier.remove((fr_r, fr_c, origin_r, origin_c))
        ...
        if is_valid(opposite_r, opposite_c) and maze[opposite_r][opposite_c] == WALL_CHAR:
            ...
            grid.set_color(fr_c, fr_r, CURRENT_COLOR)
            time.sleep(GENERATION_DELAY / 2)
            maze[fr_r][fr_c] = PATH_CHAR
            grid.set_color(fr_c, fr_r, PATH_COLOR)

            # Visualize carving the opposite cell
            grid.set_color(opposite_c, opposite_r, CURRENT_COLOR)
            time.sleep(GENERATION_DELAY)
            maze[opposite_r][opposite_c] = PATH_CHAR
            grid.set_color(opposite_c, opposite_r, PATH_COLOR)
            ...
            for nnr, nnc in get_neighbors(opposite_r, opposite_c, step=1, check_wall=True):
                ...
                if not any(f[0] == nnr and f[1] == nnc for f in frontier):
                    ...
                    # grid.set_color(nnc, nnr, FRONTIER_COLOR)
...
def generate_maze_task():
    ...
    try:
        ...
        if start_pos is None: start_pos = (0,0) # Fallback
        ...
        if end_pos is None: end_pos = (grid_height - 1, grid_width - 1) # Fallback
...
def solve_maze_dfs(start_r: int, start_c: int, end_r: int, end_c: int) -> bool:
    ...
    while stack:
        ...
        found_next = False
        # Use step=1 for solver neighbors, don't check for walls here
        for nr, nc in get_neighbors(r, c, step=1, check_wall=False):
            # Solver only cares about non-wall and unvisited
...
# ==================================
# ... (control_handler function remains the same) ...
def control_handler(msg: Dict[str, Any]):
    """Handles messages received from the Control module."""
    global solving # Declare intention to modify global variable
...
if __name__ == "__main__":
    ...
    except Exception as e: logging.exception(f"An unexpected error occurred: {e}"); console.print(f"FATAL ERROR: {e}")
    finally:
